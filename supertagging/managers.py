"""
Custom managers for Django models registered with the supertagging
application.
"""
...
class ModelTagManager(models.Manager):
    ...
    def related(self, tags, *args, **kwargs):
        return SuperTag.objects.related_for_model(tags, self.model, *args, **kwargs)
...
class ModelTaggedItemManager(models.Manager):
    ...
    def related_to(self, obj, queryset=None, num=None):
        if queryset is None:
            return SuperTaggedItem.objects.get_related(obj, self.model, num=num)
        else:
            return SuperTaggedItem.objects.get_related(obj, queryset, num=num)

    def with_all(self, tags, queryset=None):
        if queryset is None:
            return SuperTaggedItem.objects.get_by_model(self.model, tags)
        else:
            return SuperTaggedItem.objects.get_by_model(queryset, tags)

    def with_any(self, tags, queryset=None):
        if queryset is None:
            return SuperTaggedItem.objects.get_union_by_model(self.model, tags)
        else:
