...
def do_tags_for_model(parser, token):
    ...
    if len_bits == 4:
        return TagsForModelNode(bits[1], bits[3], counts=False, **kwargs)
    else:
        return TagsForModelNode(bits[1], bits[3], counts=True, **kwargs)
...
def do_tag_cloud_for_model(parser, token):
    ...
    if not len_bits > 3:
        raise TemplateSyntaxError(
            _('%s tag requires more than two arguments') % bits[0])
    if bits[2] != 'as':
        raise TemplateSyntaxError(
...
def do_relations_for_tag(parser, token):
    ...
    if len(bits) > 4:
        if bits[4] != "with":
            raise TemplateSyntaxError(_("Fouth argument for %s tag must be 'with'") % bits[0])

        if len(bits[5].split("=")) == 2 and bits[5].split("=")[0] == 'type':
            return RelationsForTagNode(bits[1], bits[3], bits[5].split("=")[1])
        else:
            raise TemplateSyntaxError(_("Last argument for %s tag must be in the format type=[TYPE]") % bits[0])
