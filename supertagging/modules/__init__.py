...
from supertagging.models import (SuperTag, SuperTagRelation, SuperTaggedItem,
                                 SuperTaggedRelationItem, SuperTagProcessQueue)
        ...
        try:
            ...
            if hasattr(result, 'entities'):
                try:
                    ...
                    if settings.ST_DEBUG:
                        raise Exception("Failed to process Entities: %s" % e)

            if hasattr(result, 'relations') and settings.PROCESS_RELATIONS:
                try:
                    _processRelations(field, result.relations, obj, ctype,
                                      proc_type, tags, date)
                except Exception, e:
                    if settings.ST_DEBUG:
                        raise Exception("Failed to process Relations: %s" % e)
            ...
            if hasattr(result, 'topics') and settings.PROCESS_TOPICS:
                try:
                    ...
                    if settings.ST_DEBUG:
                        raise Exception("Failed to process Topics: %s" % e)
            ...
            if hasattr(result, 'socialTag') and settings.PROCESS_SOCIALTAGS:
                try:
                    ...
                    if settings.ST_DEBUG:
                        raise Exception("Failed to process SocialTags: %s" % e)
